...
stonith_fence_history_cleanup(xmlNode *request, const char *target)
{
    ...
    if (remote_op_list) {
        g_hash_table_foreach_remove(remote_op_list,
...
/*!
 * \internal
 * \brief Convert xml fence-history to a hash-table like remote_op_list
 *
...
stonith_xml_history_to_list(xmlNode *history)
{
    xmlNode *xml_op = NULL;
    GHashTable *rv =
        g_hash_table_new_full(crm_str_hash, g_str_equal, NULL, free_remote_op);
...
stonith_merge_in_history_list(GHashTable *history)
{
    ...
    while (g_hash_table_iter_next(&iter, NULL, (void **)&op)) {
        remote_fencing_op_t *stored_op =
            g_hash_table_lookup(remote_op_list, op->id);
        ...
        CRM_LOG_ASSERT(g_hash_table_insert(remote_op_list, op->id, op));
    }
    if (updated) {
        do_stonith_notify(0, T_STONITH_NOTIFY_HISTORY, 0, NULL);
    }
...
stonith_fence_history(xmlNode * msg, xmlNode ** output)
{
    ...
    if (options & st_opt_cleanup) {
        ...
        if (crm_element_value(msg, F_STONITH_CALLID)) {
            /* this is coming the stonith-API
            *
